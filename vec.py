...
def equal(u, v):
    ...
    assert u.D == v.D
    if len(u.f) >= len(v.f):
        value = sum([getitem(u, k) == getitem(v, k) for k in u.f])
    else:
        value = sum([getitem(u, k) == getitem(v, k) for k in v.f])

    return max(len(u.f), len(v.f)) == value
...
def add(u, v):
    ...
    """
    assert u.D == v.D
    return Vec(v.D, {k: getitem(u, k)+getitem(v, k) for k in v.D})
...
def dot(u, v):
    ...
    assert u.D == v.D
    if len(u.f) <= len(v.f):
        value = sum([u[k]*v[k] for k in u.f])
    else:
        value = sum([u[k]*v[k] for k in v.f])
